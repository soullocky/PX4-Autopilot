...
- 使用阻尼最小二乘代替无阻尼伪逆，降低奇异位形附近的舵机抖动和解跳变。
- 对八个倾转舵机加入弱回中约束，抑制冗余解长期漂移至机械极限。
- 对输入、迭代修正量和最终状态进行有限值检查，异常时保持上一周期有效输出。
- 迭代步长仅作为非线性求解的信赖域；实际舵机速度统一由 PX4 标准 `CA_SVx_SLEW` 限制。
- 输出层限速、限幅后的真实指令会反馈给非线性模型，作为下一周期求解初值，避免模型状态与实际指令不同步。
- 使用非线性模型的实际输出回填六轴未分配控制量。
- 采用加权阻尼最小二乘：`Mx/My/Mz/Fz` 的跟踪优先级高于 `Fx/Fy`；倾转行程不足时优先保持姿态、偏航和升力。
- Tiltquad 飞行阶段不再使用 `MPC_TILTMAX_AIR` 作为矢量推力能力限制；位置控制器直接读取八个 `CA_SV_TLx_MINA/MAXA`。
- 对转换后的机体系推力进行逐轴限幅：`Fx` 受四个 Pitch-like 舵机的最小安全行程限制，`Fy` 受四个 Roll-like 舵机的最小安全行程限制；不再把双轴方形工作包络错误裁剪为单一推力锥。
- PositionControl 的合倾角限制也由同一组双轴舵机行程计算。软件将任一倾转轴限制到最多 `85°`，避免 `tan(90°)` 奇异。
- `14002_tiltquad` 默认舵机角度设为 `±20°`；实际机型可按已验证的机械无干涉范围修改 `CA_SV_TLx_MINA/MAXA`。
- 为降低 `wq:rate_ctrl` 负载，Tiltquad 非线性分配每周期仅执行一次迭代；雅可比由中央差分改为复用当前输出的前向差分，完整推力模型计算量约降低 74%。
...
- 对不可控的 `Fy` 目标不进行迭代追踪，避免不可控误差持续把舵机推向极限。
- 使用阻尼最小二乘代替无阻尼伪逆，减小秩不足和奇异位形附近的舵机抖动。
- 使用上一周期经过限速、限幅后的实际输出作为求解初值，并将迭代次数限制为两次。
- 总升力采用独立的集体电机修正，避免低推力时舵机雅可比退化造成油门无响应。
- 电机单次迭代步长限制为 `0.12`，舵机单次迭代步长限制为 `0.03`。
- 四个舵机加入弱回中约束，抑制冗余解长期漂移到 `-1/+1`。
- 矩阵求逆失败时跳过本次非线性修正，保留当前有效解。
- 舵机速度统一由 `CA_SV0_SLEW` 至 `CA_SV3_SLEW` 限制。
- `14003_single_axis_tiltquad` 明确配置四个旋翼的 `CA_ROTORx_KM` 正负号，避免错误的偏航反扭矩模型导致舵机差动饱和。
- 同步 Tiltquad 的稳定性优化：使用姿态/偏航/升力优先的加权阻尼最小二乘，倾转行程不足时优先牺牲前向 `Fx`；每周期仅一次迭代，并使用前向差分复用当前模型输出，减少 CUAV 7 Nano 的 `rate_ctrl` 负载。
- 增加有限值保护；矩阵求逆、迭代增量或执行器输出异常时保留上一周期有效指令。
- PositionControl 按四个单轴舵机中最小的 `CA_SV_TLx_MINA/MAXA` 安全行程限幅 `Fx`，避免前向矢量推力要求超过机械倾转能力；`Fy` 仍由机体 Roll 产生。
- `14003_single_axis_tiltquad` 默认倾转行程同步为 `±20°`。已有飞控上的参数不会被 `set-default` 覆盖，需按实际机械无干涉范围手动设置。
...
- `MC_AUX_ATT_TAU`：AUX5/AUX6 姿态偏置的一阶低通时间常数；默认 `0.30 s`，设为 `0` 可关闭。该滤波同时用于 Tiltquad/MCTilt 位置模式及手动/自稳模式，抑制旋钮量化导致的姿态台阶和舵机抖动；它不影响普通 Roll/Pitch 摇杆的响应。

滤波调参建议：先使用默认 `0.30 s`；仍有台阶抖动时增加至 `0.40–0.50 s`，需要更快的 AUX 姿态响应时减小至 `0.15–0.25 s`。修改后可使用 `param set MC_AUX_ATT_TAU <值>` 和 `param save` 保存。
...
- `cuav_7-nano_default.px4` 完整固件编译通过。
